...
- **Spec change (exact):** none. New files only: `portal/` (self-contained static-site builder: gate + reader + wikilink resolver + renderer + 25 tests; deps `marked`+`yaml`, lockfile-pinned; no imports from compiler `src/`), `portal/ci/portal.yml` (the deploy workflow — build+test+deploy on push to `main` only; ships outside `.github/` because the local tokens lack the `workflow` OAuth scope; one-time web-UI activation step in its header), `vitest.config.ts` (root: adds `portal/**` to vitest's default excludes so the compiler board doesn't discover the portal's node:test files — board proven identical before/after: 174 passed + 1 skipped, 12/12 files), this ledger entry.
