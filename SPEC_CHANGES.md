...
- **Status:** APPLIED (compiler — Task A + this record + the staged Task-B draft) 2026-06-08 — diff for review. **Task B vault writeback (the agent-2 prompt) is human-gated and PENDING** (after evaluator-verify + Marcia's bless; Marcia pauses Obsidian). Agent prompts are vault-only + unpinned → **no version-pin ritual, no `check-drift`/`pins.json` step.**
...
- **Review flow:** the full v0.3 is **staged for review** at `docs/SC-0035-agent-2-prompt.DRAFT.md` (banner-marked NOT canonical) so the evaluator verifies it on the remote and Marcia reads it before the writeback. On bless → transcribe to vault `_spec/agent-2-system-prompt.md` (Marcia pauses Obsidian; vault PR) → delete the staging draft.
