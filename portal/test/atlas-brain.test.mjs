...
test('brain: HUD skeleton carries the V2 controls (modes mount point, filters, search, all-threads)', { skip: !haveFixtures }, () => {
  ...
  // Naming ruling: the user-facing title is "The Meaning Ecosystem"
  // (h1 + the canvas accessible name, kept in sync); internal names
  // (atlas-brain.js, brain-on, __tripodBrain) deliberately stay brain-*.
  assert.match(index, /<h1>The Meaning Ecosystem<\/h1>/);
  assert.match(index, /aria-label="The Meaning Ecosystem — an interactive graph/);
