...
// SC-0034 — wire the SC-0031 measurement-trap helper into the gate so it RUNS, not just exists.
...
if (!hasPython3) {
  console.warn(
    "⚠ SC-0034 adversarial-check self-test SKIPPED — `python3` not found on PATH. " +
      "Install python3 to run `tools/adversarial_check.py --self-test` as part of the gate.",
  );
}

describe("SC-0034 — adversarial_check.py self-test runs in the gate (measurement-trap guard)", () => {
