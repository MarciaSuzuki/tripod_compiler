...
describe("SC-0070 slot-name role-vocabulary guard", () => {
  const slot = (mc: any) => lintMeaningCoordinates(mc).findings.filter((f) => f.rule === "slot_name_role_vocab");

  it("BLOCKS a planted banned slot-name in event_specific_slots (agent_named), tier 1", () => {
    const mc = { level_3_propositions: [{ prop_id: "P1", event_specific_slots: { agent_named: "B10", action: "PROVIDED" } }] };
    const f = slot(mc);
  ...
  it("BLOCKS a banned slot-name nested inside a *_components array", () => {
    const mc = { level_3_propositions: [{ prop_id: "P1", event_specific_slots: { vow_components: [{ recipient: "B3", speech_act: "VOWS" }] } }] };
    expect(slot(mc).some((f) => f.match === "recipient")).toBe(true);
  });

  it("flags every banned-token family (subject/object/source/locus/owner/argument/antecedent/referent/beneficiary)", () => {
    const mc = { level_3_propositions: [{ prop_id: "P1", event_specific_slots: {
    ...
    expect(slot(mc).length).toBe(9);
  });

  it("stays SILENT on the ruled keep-list (speaker / addressee / addressees)", () => {
    const mc = { level_3_propositions: [{ prop_id: "P1", event_specific_slots: { speaker: "B3", addressee: "B9", addressees: ["B8", "B9"] } }] };
    expect(slot(mc).length).toBe(0);
  });

  it("stays SILENT on the SC-0070 renamed names (named_doer / invoked_deity / blessed_party / bound_to / comparison_rank)", () => {
    const mc = { level_3_propositions: [{ prop_id: "P1", event_specific_slots: { named_doer: "B10", invoked_deity: "B10", blessed_party: "B3", bound_to: "B3", comparison_rank: "GREATER_TERM" } }] };
    expect(slot(mc).length).toBe(0);
  });

  it("does NOT false-flag referential_form via the 'referent' prefix (token match, not substring)", () => {
    const mc = { level_3_propositions: [{ prop_id: "P1", event_specific_slots: { referential_form: "HA_ISH", invoked_deity_referential_form: "YHWH" } }] };
    expect(slot(mc).length).toBe(0);
  });

  it("is scoped to event_specific_slots — never the sanctioned L2 objects_in_scene / object_id namespace", () => {
    const mc = { level_2_scenes: [{ scene_id: "S1", objects_in_scene: { entries: [{ object_id: "O1", function_in_scene: "AFFLICTION" }] } }], level_3_propositions: [] };
    expect(slot(mc).length).toBe(0);
  });
});
