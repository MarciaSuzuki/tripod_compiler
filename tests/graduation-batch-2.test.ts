 ...
 * the declared status flip draft→complete; the MEANING_COORDINATES skeletons + COMPILATION-LOGs stay `_working/`
...
describe("SC-0062 — the GENEALOGY watch-point is permanently gated", () => {
  ...
  it("P14 is GENEALOGY under NARRATIVE with the narrator constant — and validates closed-axis clean (a HOLD, not a finding)", () => {
    ...
    try {
      const mcPath = join(tmp, "P14-MEANING-COORDINATES.md");
      writeFileSync(
        mcPath,
        `---\ntype: "sta-meaning-coordinates"\npericope: "P14"\nstatus: "skeleton"\npilot: "pilot-2"\n---\n\n` +
          "```json\n" + JSON.stringify(skeleton, null, 2) + "\n```\n",
      );
      const blocks = validateArtifact(mcPath).findings.filter((f) => f.severity === "block");
