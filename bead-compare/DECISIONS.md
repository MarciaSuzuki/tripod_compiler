...
- Exceptions to "merge left": a short first run merges into its right neighbour; a short speech run whose left neighbour is a pause merges right when the right neighbour is speech, so short speech at the start of a phrase is not swallowed by the pause before it; a short speech burst between two pauses is absorbed into the left pause; a tape that is one short run stays one cluster.
...
- Because of that, resolving a carried request (on Compare, or on Listen for B) drops it from the Compare list and from the Report; its copy on B is kept and marked resolved so the trace survives in Listen and in the passage export.
...
- Carried comments are derived on every Compare/Report computation (the alignment is the source of truth) and then persisted onto B as copies with status `carried` and `carried_from` = the source id, one per source (a re-opened Compare updates in place, never duplicates). Listen on B shows them with the "carried" label and dashed markers; they cannot be deleted there, and resolving one (on B, or in Compare) resolves the source on A. A copy whose source was resolved becomes resolved; a copy whose source was deleted is removed (right away by `deleteComment`, otherwise on the next sync).
...
- Import problems are structured diagnostics (`{ code, vars }`), translated by the passage screen (`passages.import.*`); only a `detail` var carries a raw English suffix (a JSON parse error, a tape.json field check).
...
- `tape_sha256` is the SHA-256 of the canonical JSON of the parsed tape (`JSON.stringify`, mock flag only when true), not of the file's bytes, so it survives export/import; `audio_sha256` is the hash of the wav bytes. The interface labels the former "canonical JSON".
...
- A passage exports as one zip (`passage.json` manifest, `versions/<id>/{audio.wav,tape.json,meta.json}`, `comments/<id>.<ext>`), format `bead-compare-passage` version 1; the manifest keeps the exporter's ids and the importer regenerates every id, remaps `carried_from` and the pair keys, recomputes the hashes from the bytes it holds, and drops comments and pairs whose versions are not in the zip.
...
- Region colours are Okabe–Ito: orange substituted, bluish green inserted, purple-grey deleted, blue melody, so they stay apart for colour-blind readers; a carried comment that landed on a change is steel blue, "no change detected here" is vermilion (far from the orange) and is also drawn with a 2 px dashed outline so the one warning on the strip reads without colour.
...
- Numbers policy: outside `<details class="tech">` the interface shows only time positions (m:ss.s), region ordinals and counts, changed seconds and the stability percent; frame counts, frame rate, U/F series, cluster counts, alignment score and op counts, ids, hashes and the settings JSON live inside technical details, which start closed on every screen (asserted by the e2e suite).
- Beads have no visible label; their accessible name is their time position; frame indexes live only in `data-*` attributes.
- Accessible names everywhere: the settings icon button (aria-label, title, aria-haspopup), the language toggle (aria-pressed, full-name titles), player buttons (aria-label and title), the A/B radios (per version), the verdict select (screen-reader-only label), the region row (a screen-reader-only "Play region N:" prefix keeps the visible words in the name), comment markers ("kind, author, range"), the connector SVG (`role="img"`), the strips (`role="group"`), the waveform (`aria-hidden`).
- One breakpoint at 720 px collapses the version grid, region rows, report facts and the settings panel and hides the player button labels except the region player's primary one; `prefers-reduced-motion` turns transitions off; `:focus-visible` gets an accent outline; phone width is best-effort.
- The settings panel is a right-side dialog: focus moves into it, Esc closes it, values apply on every usable keystroke (Compare recomputes live), an unusable draft snaps back on blur, a comma is accepted as the decimal mark, and the panel also carries the language selector, reset and clear-all.
...
- Downloads go through a blob URL and a temporary `<a download>` revoked after 30 s; file names are ASCII-folded (Chromium drops a name with a non-ASCII character), spaces become `_`, and the report files are `bead-compare-<passage>-<A>-vs-<B>.md|json`, the passage export `<title>.zip`.
...
- "Load demo passage" is a real import of the two served fixtures, titled in the interface language.
...
- The e2e suite (Playwright) runs against the production bundle served by `vite preview` on port 4173, one fresh browser context per test, and drives everything through the UI (no writes into IndexedDB from the test); persistence is checked with `page.reload()` inside a test.
...
- `@playwright/test` is pinned to 1.56.1, whose Chromium revision matches the browser preinstalled in the build environment; a preinstalled binary is used when present, so no `playwright install` is needed there.
